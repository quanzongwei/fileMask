 ...
 * @date 2020/1/16
 */
public class SimpleEncryptionAlgorithmV4MultiBytesMappingV1 {
    ...
    private void encodeOrDecodeFile(File file, boolean isEncodeOperation) throws IOException {
        ...
        List<byte[]> randomFactor = new ArrayList<>();
        randomFactor.add(rd1.getBytes());
        randomFactor.add(rd2.getBytes());
        randomFactor.add(rd3.getBytes());
        randomFactor.add(rd4.getBytes());
        randomFactor.add(rd5.getBytes());
        randomFactor.add(rd6.getBytes());
        randomFactor.add(rd7.getBytes());
        randomFactor.add(rd8.getBytes());

        generateMap(encodeMap, decodeMap, randomFactor);
