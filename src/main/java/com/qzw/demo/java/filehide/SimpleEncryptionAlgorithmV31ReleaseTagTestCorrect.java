 ...
 * @date 2020/1/10
 */
public class SimpleEncryptionAlgorithmV31ReleaseTagTestCorrect {
     ...
     */
//    @Test
    public void test加密V3(File fileParam) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(fileParam, "rw")) {
            long t = System.currentTimeMillis();
    ...
    //uuid和长度暂时未加密
//    @Test
    public void test解密V3(File fileParam) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(fileParam, "rw")) {
            long t = System.currentTimeMillis();
    ...
    public void testCorrect() throws IOException {
        ...
        for (File file : files) {
            ...
            byte[] md5BytesWithByteInput = MD5Utils.getMd5BytesWithByteInput(bytes);
            String s = Base64.getEncoder().encodeToString(md5BytesWithByteInput);
            ...
            Long length2 = raf2.length();

            raf2.seek(0);

            byte[] bytes2 = new byte[length2.intValue()];
            raf2.read(bytes2);
            raf2.close();

            byte[] md5BytesWithByteInput2 = MD5Utils.getMd5BytesWithByteInput(bytes2);
            String s2 = Base64.getEncoder().encodeToString(md5BytesWithByteInput2);
            System.out.println(s2);

            if (!s2.equals(s)) {
                System.out.println("加密算法存在问题:" + file.getPath());
            } else {
                System.out.println("加密算法正常:" + file.getPath());
            }
        }
    }
